...
Doing so showed the characteristic ``trough'' structure of increasing relative diversity as a function of genetic distance from the focal locus under selection (model 5 shown in  \ref{fig:200kb}, see Figures \ref{fig:S912} for all models).

bneck: goes down and then up
expansion (only up)
recent bneck only down
strong bneck flatter



For the two-epoch models with a population contraction, we observed that the slope of the trough became more shallow through time (Table \X) \jri{Markus please add the table?}; this change was most apparent for the model with the strongest bottleneck (model 3), with patterns of $\pi/\pi_0$ appearing essentially flat across the 200 kb region in the final generation \ref{fig:S912}. \jri{Raul or Markus could you please fix this figure so the caption and references work?}
However,  accelerated after
