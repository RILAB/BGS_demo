...
\author[$\ast$,a]{Raul Torres}
\author[$\dagger$, $\ddagger$,a]{Markus Stetter}
\author[$\S$,1]{Ryan Hernandez}
\author[$\dagger$,$\ast \ast$,1]{Jeffrey Ross-Ibarra}

\affil[$\ast$]{Biomedical Sciences Graduate Program, University of California San Francisco, San Francisco, CA, USA}
\affil[$\dagger$]{Dept. of Plant Sciences and Center for Population Biology, University of California, Davis, CA, USA}
\affil[$\ddagger$]{Botanical Institute, University of Cologne, Cologne, Germany}
\affil[$\S$]{Department of Human Genetics, McGill University and the Genome Quebec Innovation Centre, QC, Canada}
\affil[$\ast \ast$]{Genome Center, University of California, Davis, CA, USA}
...
E-mail: ryan.hernandez@McGill.ca \\
\textsuperscript{a}{These authors contributed equally}}
\vspace{-11pt}%
...
Notably, while the rate of loss of diversity in response to a population contraction is quite fast, the recovery of diversity following population increase can be quite slow \citep{charlesworth2009effective}.
...
We find that the timing of demographic events is critical for populations experiencing non-equilibrium demography and BGS can yield contrasting patterns of diversity that reconciles apparently contradicting results  \citep{beissinger2016recent,torres2018human}.
...
Selection parameters for simulating BGS followed those of \citet{torres2018human}, with deleterious variation occurring at 20\% of sites across a 2 Mb locus and the selection coefficient, \textit{s}, drawn from a mixture of two distributions of fitness effects (DFE), representing the strength of purifying selection operating on coding and non-coding sites.
Specifically, 13\% of deleterious sites were drawn from a gamma distribution (parameterized with mean = $\alpha/\beta$, variance = $\alpha/\beta^2$) parameterized Gamma($\alpha = 0.0415$, $\beta = 80.11$) and seven percent from a distribution parameterized Gamma($\alpha = 0.184$, $\beta = 6.25$).
...
The demographic models included a constant sized population (model 1) and eleven non-equilibrium demographic models incorporating both contractions and expansions (models 2-12; Figures \ref{fig:S1}-\ref{fig:S2}; Table \ref{table:params}).
...
The first term of the equation models the effects of BGS according to the gamma DFE inferred by \citet{torgerson2009evolutionary} (represented by $\Gamma(s,\alpha_T,\beta_T)$) and the second term of the equation models the effects of BGS according to the gamma DFE inferred by \citet{boyko2008assessing} (represented by $\Gamma(s,\alpha_B,\beta_B)$).
...
In both models, both $\pi/\pi_0$ and $\xi/\xi_0$ appeared to plateau at levels above that of model the equilibrium model 1 (with $\pi/\pi_0$ approaching its expected value).
...
To compare these predictions to those of our simple demographic models, we calculated the predicted equilibrium $\pi/\pi_0$ under the classic model given the instantaneous $N_e$ at each time point.
...
Specifically,  we simulated a population undergoing a contraction similar in size to models 2 and 3, but with a subsequent exponential expansion to 400,000 individuals by the final generation (Figure \ref{fig:S2}). 
These bottleneck-expansion models included both ancient ($1.0N_{anc}$ generations in the past, models 5-8) and recent ($0.1N_{anc}$ generations in the past, models 9-12) bottlenecks as well as both instantaneous (models 5-6,9-10) or sustained bottlenecks (models 7-8,11-12).
...
Though the trajectories of $\pi/\pi_0$ and $\xi/\xi_0$ were truncated for models in which the bottleneck occurred in the recent past ($0.1N_{anc}$ generations), they nonetheless appeared to behave qualitatively similar to ancient bottleneck models (Figure \ref{fig:S912}).
...
genetic distance from the focal locus under selection (Figure 4, Figures
...
swamped any obvious patterns across the 200 kb region through time.
 Since BGS perturbs common frequency
bins of the SFS more than rare ones [57], $\xi/\xi_0$ is less affected (and, thus, closer
to 1) than $\pi/\pi_0$. Therefore, any signal using rare
...
Additionally, more rapid effects were observed for model 3 despite the fact that the expectation of $\pi/\pi_0$ for model 3 was actually higher than for model 2.
...
Importantly, these results demonstrated that classical models that predict the impact of BGS on $\pi/\pi_0$, such as the Nordborg model, implicitly assume an equilibrium population at  mutation-selection balance [58] and are inappropriate for predicting true patterns of genetic diversity for populations that have undergone recent size changes. Further, sometimes the predictions from the model can even be in the opposite direction to the initial dynamics.
...
that model. Indeed, Figure \ref{fig:nordborgsims} suggests that a threshold of $\gamma\approx0.6$ should be used.
...
experiencing selection at linked sites in maize, which underwent a demographic
...
\begin{table*}[]
\begin{center}
\caption{Demographic parameters for demographic models 1-12}
\begin{tableminipage}{\textwidth}
\begin{tabularx}{\textwidth}{l X X X X X }
\hline
Model & ancestral population size (Nanc) & bottleneck/expansion population size (Ne) & bottleneck time length (Nanc generations) & expansion time length (Nanc generations)* & final population size (Ne) \\
\hline
model 1 & 20000 &  &  &  & 20000 \\
model 2 & 20000 & 2000 & 1 &  & 2000 \\
model 3 & 20000 & 400 & 1 &  & 400 \\
model 4 & 20000 & 40000 &  & 1 & 40000 \\
...
\end{table*}
 \begin{table*}[]
 \centering
 \caption{Absolute and relative difference between final and initial generations for ξ under neutrality and BGS. Absolute difference is taken as the difference for $\xi$ under BGS or neutrality between the final generation and initial generation of each demographic model. Relative difference is taken as the quotient from dividing the final generation by the initial generation (these are the same as the measurements of the final generation in Figure S9).}
...
\begin{table*}[]
\begin{center}
\caption{Absolute and relative difference between final and initial generations for $\pi$ under neutrality and BGS. Absolute difference is taken as the difference for $\pi$ under BGS or neutrality between the final generation and initial generation of each demographic model. Relative difference is taken as the quotient from dividing the final generation by the initial generation (these are the same as the measurements of the final generation in Figure S9). We do not include differences for $\pi$ for model 4 because equilibrium does not appear to have been reached for that model.}
...
\begin{table*}[]
\begin{center}
\caption{Absolute and relative difference of $\pi$/$\pi_0$ between the last bin and first bin of the 200 kb neutral region for the initial and final generations and the generation with minimum $\pi$/$\pi_0$. Absolute difference is taken as the difference of $\pi$/$\pi_0$ between the last 10 kb bin and the first 10 kb bin of the 200 kb neutral region for the initial generation and final generation of each model’s demographic history. Relative difference is taken as the quotient from dividing $\pi$/$\pi_0$ in the last 10 kb bin by the first 10 kb bin. For models that experienced a decrease in $\pi$/$\pi_0$ followed by an increase in $\pi$/$\pi_0$ (models 2-3, 5-8), we also calculated the absolute and relative difference at the generation where minimum $\pi$/$\pi_0$ was observed in the first 10 kb bin. The first and last 10 kb bins have a genetic distance of 0.00082 cM and 0.0164 cM from the 2 Mb selected locus, respectively.}
\begin{tableminipage}{\linewidth}
